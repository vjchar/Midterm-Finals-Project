...
<section class="content-section operations-page">
    <div class="container">
        ...
        <?php if (isset($_GET["payment_submitted"])): ?>
            <div class="alert alert-success"><strong>Payment submitted.</strong> Next step: upload your required documents below.</div>
        <?php endif; ?>
        ...
        <div class="operations-grid">
            <?php foreach ($documentTypes as $type => [$label, $icon, $description]): ?>
                <?php $document = $documents[$type] ?? null; ?>
                <article class="operation-card">
                    <div class="operation-card__heading">
                        <i class="bi <?= $icon ?>"></i>
                        <div><h2><?= escape_html($label) ?></h2><p><?= escape_html($description) ?></p></div>
                        <span class="status-badge status-badge--<?= status_class($document["status"] ?? "missing") ?>"><?= escape_html(ucfirst($document["status"] ?? "missing")) ?></span>
                    </div>
                    <?php if ($document): ?>
                    ...
                    <?php endif; ?>
                    <details class="operation-upload" <?= !$document || $document["status"] === "rejected" ? " open" : "" ?>>
                        <summary><?= $document ? ($document["status"] === "rejected" ? "Replace rejected document" : "Replace document") : "Upload document" ?></summary>
                        <form method="post" enctype="multipart/form-data" class="row g-3">
                            ...
                            <div class="col-12"><button class="btn btn-primary" type="submit"><?= $document && $document["status"] === "rejected" ? "Replace Document" : "Submit for Verification" ?></button></div>
                        </form>
                    </details>
                </article>
            <?php endforeach; ?>
