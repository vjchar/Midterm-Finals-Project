...
function vehicle_delete(PDO $databaseConnection, int $vehicleId): void
{
    ...
    $bookingCountStatement = $databaseConnection->prepare(
        "SELECT COUNT(*) FROM bookings WHERE vehicle_id = :id",
    );
    $bookingCountStatement->execute(["id" => $vehicleId]);
    if ((int) $bookingCountStatement->fetchColumn() > 0) {
        throw new VehicleValidationException(
            "This vehicle has booking history and cannot be deleted.",
        );
    }
