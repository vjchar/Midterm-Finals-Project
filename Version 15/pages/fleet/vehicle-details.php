...
$requestedVehicleSlug = (string) ($_GET["vehicle"] ?? "porsche-911");
$selectedVehicle = vehicle_find($requestedVehicleSlug);
...
$interiorImageFilename = $selectedVehicle["slug"] . "-interior.png";
$hasInteriorImage = is_file(
    ROOT . "/assets/images/cars/" . $interiorImageFilename,
);
...
$availabilityMessage =
    "Select your dates to check current reservation conflicts.";
$availabilityMessageClass = "";
...
if ($availabilityPickupDate !== "" || $availabilityReturnDate !== "") {
    try {
        ...
        $availabilityPickup = new DateTimeImmutable(
            $availabilityPickupDate . " 09:00:00",
        );
        $availabilityReturn = new DateTimeImmutable(
            $availabilityReturnDate . " 09:00:00",
        );

        if ($availabilityPickup < new DateTimeImmutable("today")) {
            throw new InvalidArgumentException(
                "Pick-up cannot be in the past.",
            );
        }

        if ($availabilityReturn <= $availabilityPickup) {
            throw new InvalidArgumentException(
                "Return must be later than pick-up.",
            );
        }

        $isVehicleAvailable = vehicle_available(
            (int) $selectedVehicle["id"],
            $availabilityPickup->format("Y-m-d H:i:s"),
            $availabilityReturn->format("Y-m-d H:i:s"),
        );
        $availabilityMessage = $isVehicleAvailable
            ? "Available for the selected period."
            : "Already reserved during part of the selected period.";
        $availabilityMessageClass = $isVehicleAvailable
            ? " is-success"
            : " is-error";
    } catch (Throwable $exception) {
        $availabilityMessage = user_facing_error_message($exception);
        $availabilityMessageClass = " is-error";
    }
}

$recentVehicleSlugs = array_values(
    array_filter(
        (array) ($_SESSION["recent_vehicle_slugs"] ?? []),
        static fn(string $recentVehicleSlug): bool => $recentVehicleSlug !==
            $selectedVehicle["slug"],
    ),
);
$recentVehicleSlugs = array_slice($recentVehicleSlugs, 0, 4);
$recentlyViewedVehicles = array_values(
    array_filter(
        $vehicles,
...
$_SESSION["recent_vehicle_slugs"] = array_slice(
    array_merge([$selectedVehicle["slug"]], $recentVehicleSlugs),
    0,
    5,
);

$pageTitle = $selectedVehicle["name"] . " Details | VJ Car Rental";
$pageDescription = $selectedVehicle["description"];
...
<section class="page-hero page-hero--compact pattern-layer">
    <div class="container">
        ...
        <div class="page-hero-meta">
            <?php if ($selectedVehicle["review_count"]): ?>
                <span>
                    <i class="bi bi-star-fill"></i> <?= number_format(
                        $selectedVehicle["rating"],
                        1,
                    ) ?> from <?= (int) $selectedVehicle[
     "review_count"
 ] ?> verified review<?= $selectedVehicle["review_count"] === 1
     ? ""
     : "s" ?></span>
            <?php else: ?>
                <span>
                    <i class="bi bi-stars"></i> New vehicle - no reviews yet</span>
            <?php endif; ?>
            <span>
                <i class="bi bi-calendar-check"></i> Availability checked by rental dates</span>
        </div>
    </div>
</section>
                ...
                <div class="detail-visual pattern-layer">
                    <?php if (authenticated()): ?>
                        <form
                            ...
                            method="post"
                            action="favorite-action.php">
                            <?= csrf_field() ?>
                            <input
                                type="hidden"
                                name="vehicle"
                                value="<?= escape_html($selectedVehicle["slug"]) ?>">
                            <input
                                type="hidden"
                                name="return_to"
                                value="vehicle-details.php?vehicle=<?= escape_html(
                                    $selectedVehicle["slug"],
                                ) ?>">
                            <button
                                class="favorite-button favorite-button--large<?= $isSelectedVehicleFavorite
                                    ? " is-active"
                                    : "" ?>"
                                type="submit"
                                aria-label="<?= $isSelectedVehicleFavorite
                                    ? "Remove"
                                    : "Save" ?> <?= escape_html(
     $selectedVehicle["name"],
 ) ?> <?= $isSelectedVehicleFavorite ? "from" : "to" ?> favorites">
                                <i class="bi <?= $isSelectedVehicleFavorite
                                    ? "bi-heart-fill"
                                    : "bi-heart" ?>" aria-hidden="true"></i>
                            </button>
                        </form>
                    <?php else: ?>
                        <a
                            class="favorite-button favorite-button--large"
                            href="login.php?return_to=<?= urlencode(
                                "vehicle-details.php?vehicle=" .
                                    $selectedVehicle["slug"],
                            ) ?>"
                            aria-label="Sign in to save <?= escape_html(
                                $selectedVehicle["name"],
                            ) ?> to favorites">
                    ...
                    <?php if ($hasInteriorImage): ?>
                        <figure class="detail-interior-preview">
                            <img
                                src="assets/images/cars/<?= escape_html(
                ...
                <div class="detail-highlights detail-highlights--six">
                    <article>
                        ...
                        <span>
                            <strong><?= (int) $selectedVehicle[
                                "seats"
                            ] ?> seats</strong>
                            <small>Passenger capacity</small>
                        </span>
                    </article>
                    <article>
                ...
                <aside class="detail-booking-card">
                    <div class="detail-rating-row">
                        <?php if ($selectedVehicle["review_count"]): ?>
                            ...
                            <small><?= (int) $selectedVehicle[
                                "review_count"
                            ] ?> verified review<?= $selectedVehicle[
     "review_count"
 ] === 1
     ? ""
     : "s" ?></small>
                        <?php else: ?>
                            <span>
                                <i class="bi bi-stars"></i> New</span>
                    ...
                    <div class="booking-facts">
                        <span>
                            ...
                            <strong><?= money(
                                $selectedVehicle["deposit"],
                            ) ?></strong>
                        </span>
                        <span>
                            <small>Availability</small>
                            <strong>Checked by dates</strong>
                        </span>
                    </div>
                    <ul class="check-list">
                        <?php foreach (
                            ...
                            <li>
                                <i class="bi bi-check2-circle"></i> <?= escape_html(
                                    $inclusion,
                                ) ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                    <a class="btn btn-primary w-100" href="booking.php?vehicle=<?= urlencode(
                        $selectedVehicle["slug"],
                    ) ?>">Book This Vehicle</a>
                    <a class="btn btn-outline w-100 mt-2" href="compare.php?vehicles=<?= urlencode(
                        $selectedVehicle["slug"],
        ...
        <div class="row g-4 detail-information-row">
            <div class="col-lg-6">
                <article class="included-card included-card--expanded">
                    ...
                    <div>
                        <?php foreach (
                            $selectedVehicle["inclusions"]
                            as $inclusion
                        ): ?>
                            <span>
                                <i class="bi bi-check-circle-fill"></i> <?= escape_html(
                                    $inclusion,
                                ) ?></span>
                        <?php endforeach; ?>
                    </div>
                ...
                <article class="included-card included-card--expanded">
                    ...
                    <div>
                        <?php foreach (
                            ...
                            <span>
                                <i class="bi bi-stars"></i> <?= escape_html(
                                    $feature,
                                ) ?></span>
                        <?php endforeach; ?>
                    </div>
        ...
        <section class="availability-checker pattern-layer">
            <div>
                <span class="section-kicker">Plan your trip</span>
                <h2>Check date availability</h2>
                <p>
                    Availability is calculated from saved, active reservations for
                    <?= escape_html($selectedVehicle["name"]) ?>.
                </p>
            </div>

            <form class="availability-checker__fields" method="get" action="vehicle-details.php">
                <input
                    type="hidden"
                    name="vehicle"
                    value="<?= escape_html($selectedVehicle["slug"]) ?>">
                ...
                <button class="btn btn-primary" type="submit">Check Dates</button>
            </form>

            <p class="availability-result<?= $availabilityMessageClass ?>" role="status">
                <?= escape_html($availabilityMessage) ?>

                <?php if (($isVehicleAvailable ?? false) === true): ?>
                    <a href="booking.php?vehicle=<?= urlencode(
                        $selectedVehicle["slug"],
                    ) ?>&amp;pickup=<?= urlencode(
    $availabilityPickupDate,
) ?>&amp;return=<?= urlencode($availabilityReturnDate) ?>">
                        Continue to booking
                    </a>
                <?php endif; ?>
            </p>
            ...
            <?php else: ?>
                <div class="rating-summary-grid">
                    <article class="rating-score-card">
                        ...
                        <span>Based on <?= (int) $selectedVehicle[
                            "review_count"
                        ] ?> verified review<?= $selectedVehicle[
     "review_count"
 ] === 1
     ...
     : "s" ?></span>
                    ...
                    <div class="rating-bars">
                        <?php foreach (
                            ...
                            $score = round(
                                ((float) ($ratingAverages[$key] ?? 0)) * 20,
                            ); ?>
                            <div>
                                <span><?= escape_html($label) ?></span>
                                <progress value="<?= $score ?>" max="100"><?= $score ?>%</progress>
                                <strong><?= number_format(
                                    (float) ($ratingAverages[$key] ?? 0),
                                    1,
                                ) ?></strong>
                            </div>
                        <?php
                    ...
                    <?php foreach ($approvedReviews as $approvedReview): ?>
                        <div class="col-md-6">
                            <article class="review-card">
                                <div class="stars" aria-label="<?= (int) $approvedReview[
                                    "overall"
                                ] ?> out of 5 stars"><?= str_repeat(
     "&#9733;",
     (int) $approvedReview["overall"],
 ) . str_repeat("&#9734;", 5 - (int) $approvedReview["overall"]) ?></div>
                                <h3><?= escape_html($approvedReview["title"]) ?></h3>
                                <p><?= escape_html($approvedReview["body"]) ?></p>
                                <span>
                                    <i class="bi bi-patch-check-fill"></i> Verified rental &bull; <?= escape_html(
                                        $approvedReview["reviewer_name"],
                                    ) ?> &bull; <?= date(
     "M Y",
     strtotime($approvedReview["created_at"]),
 ) ?></span>
                            </article>
                        </div>
                    <?php endforeach; ?>
        ...
        <section class="rental-policy-grid">
            <article>
                ...
                <p><?= (int) $selectedVehicle[
                    "daily_km"
                ] ?> km is included per rental day. Excess mileage is confirmed before payment.</p>
            </article>
            <article>
        ...
        <section class="recently-viewed-section">
            <div class="section-heading">
                <span class="section-kicker">Session browsing history</span>
                <h2>Recently viewed vehicles</h2>
            </div>

            <div class="recently-viewed-grid">
                <?php if (!$recentlyViewedVehicles): ?>
                    <p class="display-note">
                        Open another vehicle to build your recently viewed list.
                    </p>
                <?php else: ?>
                    <?php foreach (
                        ...
                        <?php
                        $recentVehicleImageFilename = basename(
                            (string) $recentlyViewedVehicle["image"],
                        );
                        $recentVehicleImageRelativePath =
                            "assets/images/cars/" . $recentVehicleImageFilename;
                        $recentVehicleImageSource = is_file(
                            ROOT . "/" . $recentVehicleImageRelativePath,
                        )
                            ? $recentVehicleImageRelativePath
                            : "assets/images/placeholders/vehicle-placeholder.svg";
                        ?>
                        <a
                            ...
                            href="vehicle-details.php?vehicle=<?= urlencode(
                                $recentlyViewedVehicle["slug"],
                            ) ?>">
                            <img
                                src="<?= escape_html($recentVehicleImageSource) ?>"
                                alt="<?= escape_html(
                                    $recentlyViewedVehicle["name"],
                                ) ?>"
                                loading="lazy">
                            <span>
                                <strong><?= escape_html(
