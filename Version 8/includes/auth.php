...
function require_auth(): array
{
    $user = current_user();
    if (!$user) {
        $requestedDestination = basename($_SERVER["PHP_SELF"] ?? "index.php");
...
function require_customer(): array
{
    ...
    if ($user["role"] !== "customer") {
        ...
        require ROOT . DIRECTORY_SEPARATOR . "errors" . DIRECTORY_SEPARATOR . "403.php";
        exit();
    }
    return $user;
}
...
function password_errors(string $password): array
{
    ...
    if (strlen($password) < PASSWORD_MIN_LENGTH) {
        $errors[] = "Password must contain at least " . PASSWORD_MIN_LENGTH . " characters.";
    }
