...
$eligibleStatement = database()
    ...
    FROM bookings b JOIN vehicles v ON v.id = b.vehicle_id LEFT JOIN reviews r ON r.booking_id = b.id
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if (strlen($title) < 5 || strlen($title) > 160) {
            throw new InvalidArgumentException(
                "Use a review title between 5 and 160 characters.",
            );
        }
        if (strlen($body) < 20 || strlen($body) > 3000) {
            throw new InvalidArgumentException(
