...
<section class="page-hero page-hero--compact pattern-layer">
    <div class="container">
        ...
        <h1>My bookings</h1>
        <p>Open a reservation to review its pricing, cancel when eligible, or rate a completed trip.</p>
    </div>
</section>
                ...
                <?php foreach ($visibleBookings as $booking): ?>
                    <a class="booking-list-item"
                       href="booking-view.php?reference=<?= urlencode($booking["reference"]) ?>">
                        ...
                        <?php $bookingPayment = booking_payment_summary($booking); ?>
                        <b><?= money((int) $booking["total"]) ?><small><?= escape_html(ucfirst($bookingPayment["status"])) ?> payment</small></b>
