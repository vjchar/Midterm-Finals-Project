...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        $statement = database()->prepare(
            "SELECT id, status FROM reviews WHERE id = ? LIMIT 1",
        );
...
$pageTitle = "Review Management | VJ Car Rental";
        ...
        <div class="admin-filter-tabs">
            <?php foreach (array_merge(["all"], $validStatuses) as $status): ?>
                <a class="<?= $filter === $status
                    ? "active"
                    : "" ?>" href="review-management.php?status=<?= $status ?>"><?= ucfirst(
    $status,
) ?></a>
            <?php endforeach; ?>
        </div>
