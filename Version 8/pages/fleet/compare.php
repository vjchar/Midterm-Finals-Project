...
$pageDescription =
    "Compare VJ Car Rental vehicles by price, capacity, verified ratings, transmission, fuel, mileage, and trip fit.";
            ...
            <table class="compare-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <?php foreach ($selectedVehicles as $item): ?>
                            <td class="<?= $highestRating > 0 && (float) $item["rating"] === (float) $highestRating ? "is-best" : "" ?>">
                                <?php if ((int) $item["review_count"] > 0): ?>
                                    <span class="summary-rating"><i class="bi bi-star-fill"></i> <?= number_format((float) $item["rating"], 1) ?> / 5</span>
                                    <small><?= (int) $item["review_count"] ?> verified review<?= (int) $item["review_count"] === 1 ? "" : "s" ?></small>
                                    <?= $highestRating > 0 && (float) $item["rating"] === (float) $highestRating ? '<small class="best-label">Highest rating</small>' : "" ?>
                                <?php else: ?>
                                    <span class="summary-rating"><i class="bi bi-stars"></i> New—no reviews yet</span>
                                <?php endif; ?>
                            </td>
                        <?php endforeach; ?>
                    </tr>
