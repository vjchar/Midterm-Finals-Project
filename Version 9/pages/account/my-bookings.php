...
$visibleBookings =
    $statusFilter === "all"
        ...
        : array_values(
            array_filter(
                $bookings,
                static fn(array $booking): bool => $booking["status"] ===
                    $statusFilter,
            ),
        );
...
<section class="page-hero page-hero--compact pattern-layer">
    <div class="container">
        ...
        <h1>My bookings</h1>
        <p>Open a reservation to review its pricing, request a schedule change, cancel when eligible, or rate a completed trip.</p>
    </div>
</section>
    ...
    <div class="container">
        <div class="booking-filter-tabs" role="navigation" aria-label="Booking status filters">
            <?php foreach ($allowedStatuses as $status): ?>
                <a class="<?= $statusFilter === $status
                    ? "active"
                    : "" ?>" href="my-bookings.php?status=<?= urlencode(
    $status,
) ?>"><?= escape_html(ucfirst($status)) ?></a>
            <?php endforeach; ?>
        </div>
        <?php if (!$visibleBookings): ?>
            <div class="empty-state">
                <i class="bi bi-calendar2-x"></i>
                <h2>No <?= $statusFilter === "all"
                    ? ""
                    : escape_html($statusFilter) . " " ?>bookings found</h2>
     ...
     strtotime($booking["return_at"]),
                            ...
                            <em>
                                <i class="bi bi-geo-alt"></i> <?= escape_html(
                                    $booking["pickup_location"],
                                ) ?></em>
                        </span>
                        <b><?= money(
                            (int) $booking["total"],
                        ) ?><small>rental total</small>
                        </b>
                        <strong class="status-badge status-badge--<?= status_class(
                            $booking["status"],
                        ) ?>"><?= escape_html(ucfirst($booking["status"])) ?></strong>
